...
/**
 * Provides a friendly event driven API to litecoinzd in Node.js. Manages starting and
 * stopping litecoinzd as a child process for application support, as well as connecting
 * to multiple litecoinzd processes for server infrastructure. Results are cached in an
...
Bitcoin.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'litecoinzd/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'litecoinzd/' + name, 'total:', this.subscriptions[name].length);
};
...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'litecoinzd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'litecoinzd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Bitcoin.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('litecoinzd/hashblock', message.toString('hex'));
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('litecoinzd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('litecoinzd/rawtransaction', message.toString('hex'));
    }
